<div
    class="container max-w-67 mx-auto lg:flex lg:flex-row lg:mt-4 flex flex-col md:flex md:flex-row font-display">

    <!-- feature box -->
    <div class="lg:w-4/5 md:w-2/3 lg:h-full md:h-auto lg:ml-4">

        <!-- Breadcrumb -->
        <div class="mb-2">
            <nav class="bg-white border rounded text-light font-light" aria-label="Breadcrumb">
                <ol class="list-none p-0 lg:flex lg:flex-row p-2 flex flex-col">
                    <li class="flex items-center">
                        <a href="#" class="text-gray-500 px-2 text-light font-normal">بلاگ</a>
                        <i class="fa fa-angle-left"></i>
                    </li>
                    <li class="flex items-center">
                        <a href="#" class="text-gray-500 px-2 text-light font-normal">محصولات ارگانیک</a>
                        <i class="fa fa-angle-left"></i>
                    </li>
                    <li>
                        <a href="#" class="text-gray-500 px-2 text-light font-normal" aria-current="page">ویژگی محصولات ارگانیک تیما</a>
                    </li>
                </ol>
            </nav>

        </div>
        <!-- end Breadcrumb -->

        <!-- feature content -->
        <div class="bg-white lg:w-full lg:h-auto lg:min-h-full rounded border">
            <!-- header-->
            <div class="py-2 pr-4">
                <span class="text-gray-700 text-light font-normal">ویژگی محصولات ارگانیک تیما</span>
            </div>
            <!-- end header -->

            <hr class="border-gray-300">

            <!-- product box -->
            <div class="lg:flex lg:flex-col lg:h-auto lg:flex-row lg:mt-4 ">

                <!-- user -->
                <div class="flex lg:my-4 lg:mr-4 mb-4 mt-2 pr-8">
                    <!-- user name -->
                    <div class="w-max">
                        <i class="fa fa-user-o text-gray-400"></i>
                        <span class="text-gray-400 font-normal text-light">ناصر شرافتی نیا</span>
                    </div>
                    <!-- end user name -->
                    <!-- comment  -->
                    <div class="w-max mr-4">
                        <i class="fa fa-comment-o text-gray-400"></i>
                        <span class="text-gray-400 font-normal text-light">15</span>
                    </div>
                    <!-- end comment -->

                    <!-- date -->
                    <div class="w-max mr-4">
                        <i class="fa fa-calendar-o text-gray-400"></i>
                        <span class="text-gray-400 font-normal  text-light">1399/11/24</span>
                    </div>
                    <!-- end date -->

                </div>
                <!-- user -->

                <!-- content -->
                <div class="p-4">
                    <p class="text-gray-600 text-light font-normal">غدای ترریخته و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه مادهو محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه مادهو محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه مادهو محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه ماده
                        و محصولات طبیعی تفاوت اساسی با هم دارند. محصولات ارگانیک بدون دخالت هرگونه مادهشیمیایی و روش های مصنوعی تولید می شود. محصولات ارگانیک و مواد</p>
                </div>
                <!-- end content -->
            </div>
            <div>
                @livewire('commit')
            </div>
        </div>
        <!-- end feature content -->



    </div>
    <!-- feature box -->

    <!-- product categories -->
    <div class="bg-white lg:w-1/4 md:w-1/3 md:mx-4 lg:h-80 lg:flex lg:flex-col lg:justify-start lg:mt-0 border rounded flex flex-col justify-center w-auto h-auto mt-4
        md:mt-0 md:h-2/5 mx-2 lg:mx-0">
        <h3 class="lg:flex flex justify-center text-gray-700 text-light font-normal mt-4">دسته بندی ها</h3>
        <!-- menu -->
        <ul class="mt-6 mr-4 flex flex-col">
            <li class="hover:bg-gray-300 px-2 py-2">
                <i class="fa fa-bookmark-o text-gray-400 ml-3"></i>
                <a href="#" class="text-regular text-gray-600 hover:text-white ">محصولات ارگانیک</a>
            </li>
            <li class="hover:bg-gray-300 px-2 py-2">
                <i class="fa fa-bookmark-o text-gray-400 ml-3"></i>
                <a href="#" class="text-regular text-gray-600 hover:text-white ">کشاورزی پایدار</a>
            </li>
            <li class="hover:bg-gray-300 px-2 py-2">
                <i class="fa fa-bookmark-o text-gray-400 ml-3"></i>
                <a href="#" class="text-regular text-gray-600 hover:text-white">زعفران ارگانیک</a>
            </li>
            <li class="hover:bg-gray-300 px-2 py-2">
                <i class="fa fa-bookmark-o text-gray-400 ml-3"></i>
                <a href="#" class="text-regular text-gray-600 hover:text-white">زرشک ارگانیک</a>
            </li>
        </ul>
        <!-- end menu -->
    </div>
    <!-- end product categories -->
</div>
