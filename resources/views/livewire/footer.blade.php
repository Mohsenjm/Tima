<footer class="bottom-0 w-full h-auto bg-gray-700 h-auto font-display">

    <div class="container max-w-1200 mx-auto flex flex-col items-center lg:items-stretch h-auto">

        {{--        row 1--}}
        <div class="lg:flex lg:flex-row flex flex-col justify-center ">
            {{--        right menu--}}
            <div class="lg:w-1/3">
                <div class="flex justify-center lg:flex-col">
                    <img class="w-28 h-32 mt-4 mb-4" src="{{asset('/img/icon3.png')}}" alt="logo">
                </div>
                    <p class="text-white">مجموعه&nbsp;شرکت&nbsp;های&nbsp;تیما&nbsp;زعفران&nbsp;&nbsp;،&nbsp;بیونیک&nbsp;و&nbsp;رزلا&nbsp;با&nbsp;هدف </p>
                    <p class="text-white">توليـد&nbsp;محصـولات&nbsp;ارگانیـک&nbsp;و&nbsp;تامیــن&nbsp;محصــولات&nbsp;سالم</p>
                    <p class="text-white">بصورت&nbsp;حرفه&nbsp;ای&nbsp;فعالیت&nbsp;میکند</p>
            </div>

            {{--        center menu--}}
            <div class="lg:w-1/3">
                <div class="flex items-center text-white mr-8 mt-4">
                    <div><i class="fa fa-map-marker text-2xl"></i></div>
                    <div class="mr-8"><span class="mr-4">خراسان&nbsp;جنوبی،قاین،بلوار&nbsp;پاسداران</span></div>
                </div>
                <div class="flex items-center text-white mr-8">
                    <div><i class="fa fa-phone text-2xl"></i></div>
                    <div class="mr-8"><span class="mr-3">09155626002</span></div>
                </div>
                <div class="flex items-center text-white mr-8">
                    <div><i class="fa fa-envelope-o  text-2xl "></i></div>
                    <div class="mr-8"><span>info@timasaffron</span></div>
                </div>
            </div>

            {{--        left menu--}}
            <div class="lg:w-1/3 my-4 lg:my-0 lg:pt-4">
                <div>
                    <div>
                        <p class="mb-4 text-white mr-4 lg:mr-0">با&nbsp;وارد&nbsp;کردن&nbsp;ایمیل&nbsp;یا&nbsp;شماره&nbsp;همراه&nbsp;خود&nbsp;از
                            جدیدترین</p>
                        <p class="mb-4 text-white mr-4 lg:mr-0">&nbsp;تخفیف&nbsp;ها&nbsp;با&nbsp;خبر&nbsp;شوید</p>
                    </div>

                    <form action="#" class="lg:flex flex flex-col lg:mx-0 ml-8 mr-4">
                        <div class="lg:flex lg:flex-row lg:justify-between">
                            <input type="text" class="bg-white rounded lg:w-1/2 h-8 lg:ml-4 mb-2 w-full ">
                            <input type="text" class="bg-white rounded lg:w-1/2 h-8 w-full ">
                        </div>
                        <button type="submit"
                                class="text-center mt-4 rounded hover:bg-blue-400 h-8 w-full block bg-blue-500 text-white">ثبت
                        </button>
                    </form>
                </div>

            </div>

        </div>

        {{--        row2--}}
        <div class="flex flex-row-reverse w-full lg:flex lg:flex-row-reverse md:flex md:flex-col md:items-center">
            {{--            Electrical trust symbol --}}
            <div class="flex flex-row-reverse lg:ml-4 lg:mr-2 ml-4">
                <img class="w-20 h-20 bg-white mr-4 rounded " src="{{asset('/img/namad1.png')}}" alt="logo">
            </div>

            {{--            social icon--}}
            <div class="w-max flex  items-center mt-2 lg:ml-20 ml-12">
                <a href="#"
                   class="text-white fa fa-instagram text-2xl pl-4 px-4 py-1 hover:bg-gray-600 rounded"></a>
                <a href="#"
                   class="text-white fa fa-toggle-right text-2xl pl-4 px-4 py-1 hover:bg-gray-600 rounded"></a>

                <a href="#"
                   class="text-white fa fa-linkedin text-2xl pl-4 px-4 py-1 hover:bg-gray-600 rounded"></a>
            </div>
        </div>

        {{--        row3--}}
        <div class="flex justify-center mb-8 mt-2">
            <p class="text-white">تمامی&nbsp;حقوق&nbsp;مادی&nbsp;و&nbsp;معنوی&nbsp;نزد&nbsp;تیما&nbsp;زعفران&nbsp;محفوظ&nbsp;است</p>
        </div>
    </div>
</footer>
